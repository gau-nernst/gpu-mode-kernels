...
void kernel(
  ...
  if constexpr (DO_PROFILE) if (elect_sync()) {
    constexpr int NUM_WARPS = BLOCK_M / WARP_SIZE;
...
at::Tensor gemm_launch(
  ...
  auto this_kernel = kernel<BLOCK_M, BLOCK_N, BLOCK_K, C_N_MAJOR != SWAP_AB, NUM_STAGES, DO_PROFILE>;
  if (smem_size > 48'000)
    cudaFuncSetAttribute(this_kernel, cudaFuncAttributeMaxDynamicSharedMemorySize, smem_size);
  this_kernel<<<grid, BLOCK_M, smem_size>>>(A_tmap, B_tmap, SFA_ptr, SFB_ptr, C_ptr, new_M, new_N, K, profiler_ptr, num_entries);
...
at::Tensor gemm(
  ...
  if (false) {}
  else if (K == 16384) C = gemm_launch<128, 64, 256, true, false, 8, DO_PROFILE>(A, B, SFA, SFB, C, profiler_ptr, num_entries);
  else if (K == 7168)  C = gemm_launch< 64, 64, 512, true, false, 5, DO_PROFILE>(A, B, SFA, SFB, C, profiler_ptr, num_entries);
  else if (K == 2048)  C = gemm_launch<128, 64, 256, true, false, 6, DO_PROFILE>(A, B, SFA, SFB, C, profiler_ptr, num_entries);
  else                 C = gemm_launch<128, 64, 256, true, false, 6, DO_PROFILE>(A, B, SFA, SFB, C, profiler_ptr, num_entries);
...
def custom_kernel(data: input_t) -> output_t:
    ...
    out = profile(data[0], data[1], data[4], data[5], data[6], PROFILE_DATA, NUM_ENTRIES)
