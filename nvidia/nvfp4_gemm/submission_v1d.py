...
void kernel(
    ...
    for (int m = 0; m < 32 / 16; m++) {
      ...
      for (int i = 0; i < 16; i++) {
        ...
        if constexpr (SPLIT_K == 1) {
          ...
          reinterpret_cast<float2 *>(buf_ptr + bid_k * (M * N) + (row + 0) * N + col)[0] = float2({tmp[i * 4 + 0], tmp[i * 4 + 1]});
          reinterpret_cast<float2 *>(buf_ptr + bid_k * (M * N) + (row + 8) * N + col)[0] = float2({tmp[i * 4 + 2], tmp[i * 4 + 3]});
        }
      }
    }
  }
    ...
    for (int n = 0; n < BLOCK_N / WIDTH; n++) {
      ...
      for (int i = 0; i < WIDTH; i++) {
        ...
        if constexpr (SPLIT_K == 1)
          ...
          buf_ptr[bid_k * (M * N) + row * M + col] = tmp[i];
      }
    }
  }
  ...
  //if constexpr (true) {
    ...
    constexpr int size_per_split = (BLOCK_M * BLOCK_N) / SPLIT_K;
    constexpr int num_iters = size_per_split / (TB_SIZE * 4);
    constexpr int NUM = 8;

    auto compute_offset = [&](int idx) {
      if constexpr (C_N_MAJOR) {
        const int row = off_m + idx / BLOCK_N;
        const int col = off_n + idx % BLOCK_N;
        return row * N + col;
      } else {
        const int row = off_n + idx / BLOCK_M;
        const int col = off_m + idx % BLOCK_M;
        return row * M + col;
      }
    };

    for (int i = 0; i < num_iters; i += NUM) {
      float4 acc[NUM];
      float4 tmp[NUM];

      for (int j = 0; j < NUM; j++) {
        const int idx = bid_k * size_per_split + ((i + j) * TB_SIZE + tid) * 4;
        int offset = compute_offset(idx);
        acc[j] = __ldcs(reinterpret_cast<float4 *>(buf_ptr + offset));
      }
      ...
      for (int rank = 1; rank < SPLIT_K; rank++) {
        for (int j = 0; j < NUM; j++) {
          const int idx = bid_k * size_per_split + ((i + j) * TB_SIZE + tid) * 4;
          int offset = compute_offset(idx);
          tmp[j] = __ldcs(reinterpret_cast<float4 *>(buf_ptr + rank * (M * N) + offset));
        }
      ...
      for (int j = 0; j < NUM; j++) {
        ...
        const int idx = bid_k * size_per_split + ((i + j) * TB_SIZE + tid) * 4;
        int offset = compute_offset(idx);
        reinterpret_cast<int2 *>(C_ptr + offset)[0] = reinterpret_cast<int2 *>(out)[0];
      }
    }
  }
}
...
#define LAUNCH(K_, SPLIT_K, SWAP_AB, C_N_MAJOR, NUM_STAGES) \
  ...
  if (false) {}
  LAUNCH(16384, 2, true, true, 6) // benchmark.0
  LAUNCH( 7168, 4, true, true, 6) // benchmark.1
  LAUNCH( 2048, 2, true, true, 4) // benchmark.2
  // the rest
