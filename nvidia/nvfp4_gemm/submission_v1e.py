...
void kernel(
  ...
  // when SPLIT_K=2, warp0-1 go to CTA0, warp2-3 go to CTA1
  // when SPLIT_K=4, warp0 goes to CTA0, warp1 goes to CTA1
  //int dst_smem;
  float *dst_smem;
  if constexpr (SPLIT_K == 2) asm volatile("mapa.u64 %0, %1, %2;" : "=l"(dst_smem) : "l"(smem_ptr), "r"(warp_id / 2));
  if constexpr (SPLIT_K == 4) asm volatile("mapa.u64 %0, %1, %2;" : "=l"(dst_smem) : "l"(smem_ptr), "r"(warp_id));
    ...
    for (int m = 0; m < 32 / 16; m++) {
      ...
      for (int i = 0; i < 16; i++) {
        if constexpr (SPLIT_K == 1) {
          ...
          // "push" to destination CTA
          int row;
          if constexpr (SPLIT_K == 2) row = (bid_k * 2 + warp_id % 2) * 32 + m * 16 + lane_id / 4;
          if constexpr (SPLIT_K == 4) row = bid_k * 32 + m * 16 + lane_id / 4;
          const int col = i * 8 + (lane_id % 4) * 2;

          reinterpret_cast<float2 *>(dst_smem + (row + 0) * BLOCK_N + col)[0] = float2({tmp[i * 4 + 0], tmp[i * 4 + 1]});
          reinterpret_cast<float2 *>(dst_smem + (row + 8) * BLOCK_N + col)[0] = float2({tmp[i * 4 + 2], tmp[i * 4 + 3]});

          //int addr0 = dst_smem + ((row + 0) * BLOCK_N + col) * (int)sizeof(float);
          //int addr1 = dst_smem + ((row + 8) * BLOCK_N + col) * (int)sizeof(float);
          //asm volatile("st.shared::cluster.v2.f32 [%0], {%1, %2};" :: "r"(addr0), "f"(tmp[i * 4 + 0]), "f"(tmp[i * 4 + 1]));
          //asm volatile("st.shared::cluster.v2.f32 [%0], {%1, %2};" :: "r"(addr1), "f"(tmp[i * 4 + 2]), "f"(tmp[i * 4 + 3]));
        }
      }
    }
  }
    ...
    for (int n = 0; n < BLOCK_N / WIDTH; n++) {
      ...
      for (int i = 0; i < WIDTH; i++) {
        if constexpr (SPLIT_K == 1) {
          ...
          // "push" to destination CTA
          const int row = n * WIDTH + i;
          int col;
          if constexpr (SPLIT_K == 2) col = (bid_k * 2 + warp_id % 2) * 32 + lane_id;
          if constexpr (SPLIT_K == 4) col = bid_k * 32 + lane_id;
          dst_smem[row * BLOCK_M + col] = tmp[i];

          //int addr = dst_smem + (row * BLOCK_M + col) * (int)sizeof(float);
          //asm volatile("st.shared::cluster.f32 [%0], %1;" :: "r"(addr), "f"(tmp[i]));
        }
      }
    }
  }
  ...
  float *ptr = reinterpret_cast<float *>(smem_ptr);

  auto load_f32x4 = [](float *tmp, int addr) {
  ...
  constexpr int NUM = 1;
  for (int i = 0; i < 128 / SPLIT_K / NUM_WARPS; i += NUM) {
    // this logic is wrong for C M-major (split along columns)
    //float acc[4];
    float4 acc[NUM];
    float4 tmp[NUM];
    for (int j = 0; j < NUM; j++)
      acc[j] = reinterpret_cast<float4 *>(ptr + ((i + j) * NUM_WARPS + warp_id) * 128 + lane_id * 4)[0];
    //load_f32x4(acc, smem + ((i * NUM_WARPS + warp_id) * 128 + lane_id * 4) * (int)sizeof(float));

    for (int rank = 1; rank < SPLIT_K; rank++) {
      for (int j = 0; j < NUM; j++)
        tmp[j] = reinterpret_cast<float4 *>(ptr + (rank * (128 / SPLIT_K) + (i + j) * NUM_WARPS + warp_id) * 128 + lane_id * 4)[0];
      for (int j = 0; j < NUM; j++) {
        acc[j].x += tmp[j].x;
        acc[j].y += tmp[j].y;
        acc[j].z += tmp[j].z;
      ...
      //float tmp[4];
      //load_f32x4(tmp, smem + ((rank * (128 / SPLIT_K) + i * NUM_WARPS + warp_id) * 128 + lane_id * 4) * (int)sizeof(float));
      //for (int j = 0; j < 4; j++)
      //  acc[j] += tmp[j];
    }
    ...
    for (int j = 0; j < NUM; j++) {
      half2 out[2];
      out[0] = __float22half2_rn({acc[j].x, acc[j].y});
      out[1] = __float22half2_rn({acc[j].z, acc[j].w});
      //out[0] = __float22half2_rn({acc[0], acc[1]});
      //out[1] = __float22half2_rn({acc[2], acc[3]});
      const int row = bid_k * (128 / SPLIT_K) + (i + j) * NUM_WARPS + warp_id;
      const int col = lane_id * 4;
      if constexpr (C_N_MAJOR)
        reinterpret_cast<int2 *>(C_ptr + (off_m + row) * N + (off_n + col))[0] = reinterpret_cast<int2 *>(out)[0];
      else
        reinterpret_cast<int2 *>(C_ptr + (off_n + row) * M + (off_m + col))[0] = reinterpret_cast<int2 *>(out)[0];
    }
  }
}
...
at::Tensor gemm_launch(
  ...
  auto this_kernel = kernel<K, BLOCK_K, SPLIT_K, C_N_MAJOR != SWAP_AB, NUM_STAGES>;
  if (smem_size > 48'000)
    cudaFuncSetAttribute(this_kernel, cudaFuncAttributeMaxDynamicSharedMemorySize, smem_size);
  this_kernel<<<grid, TB_SIZE, smem_size>>>(A_tmap, B_tmap, SFA_ptr, SFB_ptr, buf_ptr, C_ptr, new_M, new_N);
...
#define LAUNCH(K_, SPLIT_K, SWAP_AB, C_N_MAJOR, NUM_STAGES) \
  else if (K == K_) C = gemm_launch<K_, 256, SPLIT_K, SWAP_AB, C_N_MAJOR, NUM_STAGES>(A, B, SFA, SFB, buf, C);

  if (false) {}
  LAUNCH(16384, 2, false, true, 6) // benchmark.0
  LAUNCH( 7168, 4, false, true, 6) // benchmark.1
  LAUNCH( 2048, 2, false, true, 4) // benchmark.2
  // the rest
  LAUNCH( 256, 1, true, true, 4)
  LAUNCH( 512, 1, true, true, 4)
  LAUNCH(1536, 1, true, true, 4)
  LAUNCH(2304, 1, true, true, 4)
...
def custom_kernel(data: input_t) -> output_t:
    ...
    return gemm(data[0], data[1], data[4], data[5], allocate(data[6]), data[6])
