...
#define PTX_MOD ".nc.L1::no_allocate"
  asm volatile("ld.global" PTX_MOD ".v2.b16 {%0, %1}, [%2];\n" : "=h"(dst[0]), "=h"(dst[1]) : "l"(src));
#undef PTX_MOD
}
...
//# define PTX_MOD ".cs"
//# define PTX_MOD ".L1::no_allocate.L2::evict_first"
//# define PTX_MOD ".cs.nc"
# define PTX_MOD ".nc.L1::no_allocate.L2::evict_first"
  asm volatile("ld.global" PTX_MOD ".v8.b32 {%0, %1, %2, %3, %4, %5, %6, %7}, [%8];\n"
...
//#define PTX_MOD ".ca"
//#define PTX_MOD ".L1::evict_last.L2::evict_last"
//#define PTX_MOD ".ca.nc"
#define PTX_MOD ".nc.L1::evict_last.L2::evict_last"
  asm volatile("ld.global" PTX_MOD ".v8.b32 {%0, %1, %2, %3, %4, %5, %6, %7}, [%8];\n"
              : "=r"(dst[0]), "=r"(dst[1]), "=r"(dst[2]), "=r"(dst[3]),
                "=r"(dst[4]), "=r"(dst[5]), "=r"(dst[6]), "=r"(dst[7])
              : "l"(src));
...
void kernel(
  ...
  static_assert(BLOCK_K % 32 == 0);  // each thread reads 16 bytes
  ...
  constexpr int num_cols = BLOCK_K / 32;  // each thread reads 32-byte at a time
  constexpr int TB_HEIGHT = TB_SIZE / TB_WIDTH;

  // for gmem->rmem
  int A_rmem[BLOCK_M / TB_HEIGHT][num_cols / TB_WIDTH][8];
  int B_rmem[num_cols / TB_WIDTH][8];
  int16_t SFA_rmem[BLOCK_M / TB_HEIGHT][num_cols / TB_WIDTH][2];
  int16_t SFB_rmem[num_cols / TB_WIDTH][2];
  ...
  auto gmem_to_rmem = [&]() {
    for (int k = 0; k < num_cols / TB_WIDTH; k++) {
      ...
      ldca_i32x8(B_rmem[k], B_ptr + (col * 32));
      ldca_i16x2(SFB_rmem[k], SFB_ptr + (col * 4));
    }
    ...
    for (int m = 0; m < BLOCK_M / TB_HEIGHT; m++)
      for (int k = 0; k < num_cols / TB_WIDTH; k++) {
        ...
        const int col = k * TB_WIDTH + (tid % TB_WIDTH);
        ldcs_i32x8(A_rmem[m][k], A_ptr + row * K + (col * 32));
        ldcs_i16x2(SFA_rmem[m][k], SFA_ptr + row * (K / 8) + (col * 4));
      }
  ...
  auto compute = [&]() {
    ...
    half2 B_fp16x2[num_cols / TB_WIDTH][32];
    half2 SFB_fp16x2[num_cols / TB_WIDTH][2];

    for (int k = 0; k < num_cols / TB_WIDTH; k++) {
      for (int i = 0; i < 8; i++)
        fp4x8_to_fp16x2x4(B_fp16x2[k] + i * 4, B_rmem[k][i]);
      fp8x2_to_fp16x2(SFB_fp16x2[k] + 0, SFB_rmem[k][0]);
      fp8x2_to_fp16x2(SFB_fp16x2[k] + 1, SFB_rmem[k][1]);
    }

    for (int m = 0; m < BLOCK_M / TB_HEIGHT; m++)
      for (int k = 0; k < num_cols / TB_WIDTH; k++) {
        half2 SFA_fp16x2[2];
        fp8x2_to_fp16x2(SFA_fp16x2 + 0, SFA_rmem[m][k][0]);
        fp8x2_to_fp16x2(SFA_fp16x2 + 1, SFA_rmem[m][k][1]);

        half2 A_fp16x2[16];
        half2 acc[2];

        for (int i = 0; i < 4; i++)
          fp4x8_to_fp16x2x4(A_fp16x2 + i * 4, A_rmem[m][k][i]);

        __half2_raw scales01 = __hmul2(SFA_fp16x2[0], SFB_fp16x2[k][0]);
        acc[0] = __hmul2(A_fp16x2[0], B_fp16x2[k][0]);  // 1st group
        acc[1] = __hmul2(A_fp16x2[8], B_fp16x2[k][8]);  // 2nd group
        for (int i = 1; i < 8; i++) {
          acc[0] = __hfma2(A_fp16x2[0 + i], B_fp16x2[k][0 + i], acc[0]);  // 1st group
          acc[1] = __hfma2(A_fp16x2[8 + i], B_fp16x2[k][8 + i], acc[1]);  // 2nd group
        }
        __half_raw group0 = __hadd(acc[0].x, acc[0].y);
        __half_raw group1 = __hadd(acc[1].x, acc[1].y);
        asm volatile("fma.rn.f32.f16 %0, %1, %2, %0;" : "+f"(master_acc[m]) : "h"(group0.x), "h"(scales01.x));
        asm volatile("fma.rn.f32.f16 %0, %1, %2, %0;" : "+f"(master_acc[m]) : "h"(group1.x), "h"(scales01.y));

        for (int i = 0; i < 4; i++)
          fp4x8_to_fp16x2x4(A_fp16x2 + i * 4, A_rmem[m][k][4+i]);

        __half2_raw scales23 = __hmul2(SFA_fp16x2[1], SFB_fp16x2[k][1]);
        acc[0] = __hmul2(A_fp16x2[0], B_fp16x2[k][16]);  // 3rd group
        acc[1] = __hmul2(A_fp16x2[8], B_fp16x2[k][24]);  // 4th group
        for (int i = 1; i < 8; i++) {
          acc[0] = __hfma2(A_fp16x2[0 + i], B_fp16x2[k][16 + i], acc[0]);  // 3rd group
          acc[1] = __hfma2(A_fp16x2[8 + i], B_fp16x2[k][24 + i], acc[1]);  // 4th group
        }
        __half_raw group2 = __hadd(acc[0].x, acc[0].y);
        __half_raw group3 = __hadd(acc[1].x, acc[1].y);
        asm volatile("fma.rn.f32.f16 %0, %1, %2, %0;" : "+f"(master_acc[m]) : "h"(group2.x), "h"(scales23.x));
        asm volatile("fma.rn.f32.f16 %0, %1, %2, %0;" : "+f"(master_acc[m]) : "h"(group3.x), "h"(scales23.y));
      }
  };
...
#define launch(BLOCK_M, BLOCK_K, TB_WIDTH, NUM_WARPS) { \
  ...
  if (false) {}
  else if (K == 8192) launch(8, 1024, 32, 4)  // benchmark.0
  else if (K == 3584) launch(8, 512, 16, 2)   // benchmark.1
  else if (K == 1024) launch(8, 1024, 32, 4)  // benchmark.2
  else launch(32, 128, 4, 4)                  // the rest
