...
void kernel(
  ...
  const int off_m = (bid * BLOCK_M) + (tid / THREAD_K) * THREAD_M;
  ...
  for (int iter_k = 0; iter_k < num_iters; iter_k ++) {
    ...
    for (int m = 0; m < THREAD_M; m++) {
      asm volatile("ld.global.v4.u32.cs {%0, %1, %2, %3}, [%4];\n"
                  : "=r"(A_fp4x8[m][0]), "=r"(A_fp4x8[m][1]), "=r"(A_fp4x8[m][2]), "=r"(A_fp4x8[m][3])
                  : "l"(A_ptr + m * K));
      SFA_fp32x2[m] = static_cast<float2>(reinterpret_cast<const __nv_fp8x2_e4m3 *>(SFA_ptr + m * (K / 8))[0]);
    }

    reinterpret_cast<int4 *>(B_fp4x8)[0] = reinterpret_cast<const int4 *>(B_ptr)[0];
    SFB_fp32x2 = static_cast<float2>(reinterpret_cast<const __nv_fp8x2_e4m3 *>(SFB_ptr)[0]);
  ...
  if (tid % THREAD_K == 0) {
    ...
    reinterpret_cast<int2 *>(C_ptr + (batch_id * M + off_m))[0] = reinterpret_cast<int2 *>(out)[0];
  }
}
