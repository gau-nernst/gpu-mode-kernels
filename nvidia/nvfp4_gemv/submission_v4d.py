...
void kernel(
  ...
  Profiler profiler;
  if constexpr (DO_PROFILE) if (tid == 0 || tid == WARP_SIZE)
    profiler.init(profiler_ptr, (batch_id * gridDim.x + bid) * 2 + (tid / WARP_SIZE));
  ...
  if (warp_id == 0 && elect_one_sync()) {
    ...
    for (int iter_k = 0; iter_k < num_iters; iter_k++) {
      ...
      mbarrier_wait(fma_mbar_addrs[stage_id], phase);
      prof_stop();
      ...
      for (int row = 0; row < BLOCK_M; row++) {
        cp_async_bulk(  A_smem_u32 + stage_id *   A_size + row *    BLOCK_K,   A_ptr + row *       K,    BLOCK_K, mbar_addr);
        cp_async_bulk(SFA_smem_u32 + stage_id * SFA_size + row * SF_BLOCK_K, SFA_ptr + row * (K / 8), SF_BLOCK_K, mbar_addr);
      }
  ...
  // compute warps
    ...
    const int tid_ = tid - WARP_SIZE;  // remove DMA warp
    int phase = 0;

    // to be used for smem->rmem load
    const int off_k = (tid_ % THREAD_K) * 16;  // each thread reads 16 fp4x2 values at a time

    char   *A_smem_ld =   A_smem + (tid_ / THREAD_K) * THREAD_M *    BLOCK_K + off_k;
    char   *B_smem_ld =   B_smem                                            + off_k;
    char *SFA_smem_ld = SFA_smem + (tid_ / THREAD_K) * THREAD_M * SF_BLOCK_K + (off_k / 8);
    ...
    if constexpr (THREAD_K > WARP_SIZE) {
      ...
      for (int i = 0; i < THREAD_M / 4; i++)
        reinterpret_cast<float4 *>(red_smem)[i * TB_SIZE + tid_] = reinterpret_cast<float4 *>(acc_fp32x2)[i];
      __syncthreads();

      for (int stride = THREAD_K / 2; stride >= WARP_SIZE; stride /= 2) {
        if ((tid_ % THREAD_K) < stride) {
          for (int i = 0; i < THREAD_M / 4; i++) {
            int64_t tmp[2];

            // 16-byte load
            reinterpret_cast<float4 *>(tmp)[0] = reinterpret_cast<float4 *>(red_smem)[i * TB_SIZE + (tid_ + stride)];
    ...
    if (tid_ % THREAD_K == 0) {
      ...
      half *out_ptr = C_ptr + (batch_id * M) + (bid * BLOCK_M) + (tid_ / THREAD_K) * THREAD_M;
...
#define launch(THREAD_M, THREAD_K, NUM_STAGES) { \
  ...
  else if (K == 3584) launch(4,  32, 3)  // benchmark.1
  else if (K == 1024) launch(4,  64, 2)  // benchmark.2
