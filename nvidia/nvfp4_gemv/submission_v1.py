...
__device__
void fp4x8_to_fp32x8(int in, float *out) {
  int tmp[4];
  asm volatile(
    "{\n"
...
void kernel(
  ...
  for (int iter_k = 0; iter_k < num_iters; iter_k ++) {
    auto A_fp4x32 = reinterpret_cast<const int4 *>(A_ptr)[0];
    auto B_fp4x32 = reinterpret_cast<const int4 *>(B_ptr)[0];
    ...
    for (int group_id = 0; group_id < 2; group_id++) {
      ...
      for (int i = 0; i < 2; i++) {
        ...
        fp4x8_to_fp32x8(reinterpret_cast<int *>(&A_fp4x32)[group_id * 2 + i], A_vals);
        fp4x8_to_fp32x8(reinterpret_cast<int *>(&B_fp4x32)[group_id * 2 + i], B_vals);
        for (int j = 0; j < 8; j++)
          sub_acc += A_vals[j] * B_vals[j];
      }
