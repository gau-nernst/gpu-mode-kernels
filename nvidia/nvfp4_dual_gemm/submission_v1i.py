...
void mbarrier_arrive_expect_tx(int mbar_addr, int size) {
  asm volatile("mbarrier.arrive.expect_tx.release.cta.shared::cta.b64 _, [%0], %1;" :: "r"(mbar_addr), "r"(size) : "memory");
}
...
template <int K, int BLOCK_N, int NUM_STAGES, int MCAST_A>
__global__
__cluster_dims__(MCAST_A, 1, 1)  // MCAST_A corresponds to N-dim
...
void kernel_cutlass (
  ...
  const int ctaid_n = bid_n % MCAST_A;
  ...
  if (warp_id == 0 && elect_sync()) {
    ...
    for (int i = 0; i < NUM_STAGES; i++) {
      ...
      mbarrier_init(mma_mbar_addr + i * 8, 2);  // wait for 2 CTA ranks
    }
  ...
  int all_A_smem = smem;
  int all_B1_smem = all_A_smem + NUM_STAGES * A_size;
  int all_B2_smem = all_B1_smem + NUM_STAGES * B_size;
  int all_SFA_smem = all_B2_smem + NUM_STAGES * B_size;
  int all_SFB1_smem = all_SFA_smem + NUM_STAGES * SF_size;
  int all_SFB2_smem = all_SFB1_smem + NUM_STAGES * SF_size;
  ...
  if (warp_id == NUM_WARPS - 2) {
    ...
    if (elect_sync()) {
      ...
      // adjust offsets based on CTA rank within a cluster
      const uint16_t a_tma_mask = (1 << MCAST_A) - 1;
      all_A_smem += ctaid_n * (A_size / MCAST_A);
      const int off_m_a = off_m + ctaid_n * (BLOCK_M / MCAST_A);

      // 512 = 32x4x4
      all_SFA_smem += ctaid_n * (SF_size / MCAST_A);
      const char *SFA_src = SFA_ptr + bid_m * rest_k * 512 + ctaid_n * (SF_size / MCAST_A);
      ...
      for (int iter_k = 0; iter_k < num_iters; iter_k++) {
        ...
        const int B1_smem = all_B1_smem + stage_id * B_size;
        const int B2_smem = all_B2_smem + stage_id * B_size;
        const int SFA_smem = all_SFA_smem + stage_id * SF_size;
        const int SFB1_smem = all_SFB1_smem + stage_id * SF_size;
        const int SFB2_smem = all_SFB2_smem + stage_id * SF_size;

        // wait MMA
        mbarrier_wait(mma_mbar_addr + stage_id * 8, mma_phase);

        // issue TMA
        tma_3d_gmem2smem_mcast(A_smem, &A_tmap, 0, off_m_a, iter_k, mbar_addr, a_tma_mask, cache_A);
        ...
        tma_gmem2smem_mcast(SFA_smem, SFA_src + iter_k * 2048, SF_size / MCAST_A, mbar_addr, a_tma_mask, cache_A);
    ...
    if (elect_sync()) {
      ...
      const uint16_t commit_mask = (1 << MCAST_A) - 1;
      ...
      for (int iter_k = 0; iter_k < num_iters; iter_k++) {
        ...
        int scale_B1_tmem = scale_A_tmem + 4 * (BLOCK_K / MMA_K);
        int scale_B2_tmem = scale_B1_tmem + 4 * (BLOCK_K / MMA_K);
        ...
        tcgen05_commit_mcast(mma_mbar_addr + stage_id * 8, commit_mask);  // signal MMA done
        stage_id = (stage_id + 1) % NUM_STAGES;
        if (stage_id == 0)
          tma_phase ^= 1;
      }
...
template <int K, int BLOCK_N, int MCAST_A>
void dual_gemm_launch(
  ...
  init_AB_tmap(&A_tmap, A_ptr, M, K, BLOCK_M / MCAST_A, BLOCK_K);
  init_AB_tmap(&B1_tmap, B1_ptr, N, K, BLOCK_N, BLOCK_K);
  init_AB_tmap(&B2_tmap, B2_ptr, N, K, BLOCK_N, BLOCK_K);

  dim3 grid(N / BLOCK_N, M / BLOCK_M);

  constexpr int AB_size = (BLOCK_M + BLOCK_N * 2) * (BLOCK_K / 2);
  constexpr int SF_size = 128 * (BLOCK_K / 16) * (1 + 2);

  // we use (2 * NUM_STAGES + 1) mbars, each is 8-byte.
  // 4 is size of tmem address.
  constexpr int sm100_size = 227'000;
  constexpr int NUM_STAGES = (sm100_size - 4 - 8) / (AB_size + SF_size + 8 * 2);

  constexpr int mbar_size = (2 * NUM_STAGES + 1) * 8;
  constexpr int smem_size = (AB_size + SF_size) * NUM_STAGES + mbar_size + 4;

  // cutlass incantation (this affects ptxas)
  auto this_kernel = kernel_cutlass<K, BLOCK_N, NUM_STAGES, MCAST_A>;
...
#define LAUNCH(K_, BLOCK_N) \
  dual_gemm_launch<K_, BLOCK_N, 2>(A_ptr, B1_ptr, B2_ptr, SFA_ptr, SFB1_ptr, SFB2_ptr, C_ptr, M, N);
