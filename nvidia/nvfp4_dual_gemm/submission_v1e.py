...
void kernel (
  ...
  if (warp_id == NUM_WARPS - 2) {
    ...
    for (int iter_k = 0; iter_k < num_iters; iter_k++) {
      ...
      uint64_t sfb1_desc = SF_desc + ((uint64_t)SFB1_smem >> 4ULL);
      uint64_t sfb2_desc = SF_desc + ((uint64_t)SFB2_smem >> 4ULL);
      ...
      for (int k = 0; k < BLOCK_K / MMA_K; k++) {
        ...
        // go to the next 512-byte (128x4 = 32x4x4)
        sfa_desc += (512 >> 4);
        sfb1_desc += (512 >> 4);
        sfb2_desc += (512 >> 4);
      }

      uint64_t a_desc = AB_desc + ((uint64_t)A_smem >> 4ULL);
      uint64_t b1_desc = AB_desc + ((uint64_t)B1_smem >> 4ULL);
      uint64_t b2_desc = AB_desc + ((uint64_t)B2_smem >> 4ULL);
      ...
      for (int k1 = 0; k1 < BLOCK_K / 256; k1++) {
        ...
        // go to the next (BLOCK_M, 256). need to revert the previous 128B.
        a_desc += (BLOCK_M - 1) * (128 >> 4);
        b1_desc += (BLOCK_N - 1) * (128 >> 4);
        b2_desc += (BLOCK_N - 1) * (128 >> 4);
      }
