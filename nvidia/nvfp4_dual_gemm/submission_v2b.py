...
template <int K, int BLOCK_N, int NUM_STAGES, int MCAST_B>
__global__
__cluster_dims__(2 * MCAST_B, 1, 1)
...
void kernel_cutlass (
  ...
  constexpr int CLUSTER_SIZE = 2 * MCAST_B;
  const int cta_rank = bid_m % CLUSTER_SIZE;
  const int cta_parity = bid_m % 2;
  ...
  if (warp_id == 0 && elect_sync()) {
    ...
    for (int i = 0; i < NUM_STAGES; i++) {
      mbarrier_init(tma_mbar_addr + i * 8, 2);        // each even-odd CTA pair report to their even CTA
      mbarrier_init(mma_mbar_addr + i * 8, MCAST_B);  // MMA-issuing (even) CTAs within a cluster report to everyone in the cluster
    }
    mbarrier_init(mainloop_mbar_addr, 1);  // MMA-issuing (even) CTAs report to their even-odd CTA pair
  ...
  const int all_A_smem = smem;
  const int all_B1_smem = all_A_smem + A_size * NUM_STAGES;
  const int all_B2_smem = all_B1_smem + (B_size / 2) * NUM_STAGES;
  const int all_SFA_smem = all_B2_smem + (B_size / 2) * NUM_STAGES;
  const int all_SFB_smem = all_SFA_smem + SF_size * NUM_STAGES;
  ...
  if (warp_id == NUM_WARPS - 2) {
    ...
    if (elect_sync()) {
      ...
      // mcast within even CTAs (0b0101) or odd CTAs (0b1010)
      // (only used when MCAST_B > 1)
      const int16_t b_mask = 0b101 << cta_parity;

      // even CTAs load 1st half of B, odd CTAs load 2nd half of B
      // for each half of B, we divide it into MCAST_B splits
      const int b_mcast_off_n = (cta_rank / 2) * (BLOCK_N / CLUSTER_SIZE);

      // select source slice in gmem
      const int b_off_n = off_n + cta_parity * (BLOCK_N / 2) + b_mcast_off_n;

      // select destination slice in smem
      const int all_B1_smem_ = all_B1_smem + b_mcast_off_n * (BLOCK_K / 2);
      const int all_B2_smem_ = all_B2_smem + b_mcast_off_n * (BLOCK_K / 2);

      // -- setup TMA load for SFB --
      // mcast to all CTAs in the cluster
      constexpr int16_t sfb_mask = (1 << CLUSTER_SIZE) - 1;

      // first MCAST_B CTAs load SFB1, next MCAST_B CTAs load SFB2
      // e.g. MCAST_B=1 -> CTA0      load SFB1, CTA1      load SFB2
      //      MCAST_B=2 -> CTA0+CTA1 load SFB1, CTA2+CTA3 load SFB2
      auto SFB_tmap_ptr = (cta_rank / MCAST_B) == 0 ? &SFB1_tmap : &SFB2_tmap;

      // for each SFB1/SFB2, we divide it into MCAST_B splits
      const int sfb_mcast_off = (cta_rank % MCAST_B) * (SF_size / MCAST_B);

      // select destination slice in smem
      const int all_SFB_smem_ = all_SFB_smem + cta_rank * (SF_size / MCAST_B);

      // don't unroll. this is important
      #pragma unroll 1
      for (int iter_k = 0; iter_k < num_iters; iter_k++) {
        // select tma mbar and smem
        const int mbar_addr = (tma_mbar_addr + stage_id * 8) & 0xFEFFFFFF;  // even CTA
        const int A_smem = all_A_smem + stage_id * A_size;
        const int B1_smem = all_B1_smem_ + stage_id * (B_size / 2);
        const int B2_smem = all_B2_smem_ + stage_id * (B_size / 2);
        const int SFA_smem = all_SFA_smem + stage_id * SF_size;
        const int SFB_smem = all_SFB_smem_ + stage_id * (SF_size * 2);

        // wait MMA
        mbarrier_wait(mma_mbar_addr + stage_id * 8, mma_phase);

        // issue TMA
        if constexpr (MCAST_B == 1) {
          // no mcast
          ...
          // w/ mcast
          tma_3d_gmem2smem_mcast<2>(B1_smem, &B1_tmap, 0, b_off_n, iter_k, mbar_addr, b_mask, cache_B);
          tma_3d_gmem2smem_mcast<2>(B2_smem, &B2_tmap, 0, b_off_n, iter_k, mbar_addr, b_mask, cache_B);
        }
        ...
        const int off_sfb = (off_n / 128) * rest_k * 512 + iter_k * 2048 + sfb_mcast_off;
        const int off_sfa = bid_m * rest_k * 512 + iter_k * 2048;
        tma_1d_gmem2smem_mcast<2>(SFB_smem, SFB_tmap_ptr, off_sfb / 8, mbar_addr, sfb_mask, cache_B);
    ...
    constexpr int16_t mma_mask = (1 << CLUSTER_SIZE) - 1;  // everyone in the cluster

    // only even CTAs issues MMA
    if (cta_parity == 0 && elect_sync()) {
      int stage_id = 0;
      int tma_phase = 0;
      ...
      for (int iter_k = 0; iter_k < num_iters; iter_k++) {
        // select smem
        const int A_smem = all_A_smem + stage_id * A_size;
        const int B1_smem = all_B1_smem + stage_id * (B_size / 2);
        const int B2_smem = all_B2_smem + stage_id * (B_size / 2);
        const int SFA_smem = all_SFA_smem + stage_id * SF_size;
        const int SFB1_smem = all_SFB_smem + stage_id * (SF_size * 2);
        ...
        // signal MMA done for everyone in the cluster
        tcgen05_commit_mcast<2>(mma_mbar_addr + stage_id * 8, mma_mask);
        stage_id = (stage_id + 1) % NUM_STAGES;
        if (stage_id == 0)
          tma_phase ^= 1;
      }

      // signal mainloop done its own even-odd CTA pair
      const int16_t mainloop_mask = 3 << (cta_rank / 2 * 2);
      tcgen05_commit_mcast<2>(mainloop_mbar_addr, mainloop_mask);
    }
  }
  else {
    // epilogue warps
...
template <int K, int BLOCK_N, int MCAST_B>
void dual_gemm_launch(
  ...
  init_AB_tmap(&B1_tmap, B1_ptr, N, K, BLOCK_N / 2 / MCAST_B, BLOCK_K);
  init_AB_tmap(&B2_tmap, B2_ptr, N, K, BLOCK_N / 2 / MCAST_B, BLOCK_K);
  ...
  init_SF_tmap(&SFB1_tmap, SFB1_ptr, N * K / 16, 128 * BLOCK_K / 16 / MCAST_B);
  init_SF_tmap(&SFB2_tmap, SFB2_ptr, N * K / 16, 128 * BLOCK_K / 16 / MCAST_B);
  ...
  auto this_kernel = kernel_cutlass<K, BLOCK_N, NUM_STAGES, MCAST_B>;
...
#define LAUNCH(K_, BLOCK_N, MCAST_B) \
  dual_gemm_launch<K_, BLOCK_N, MCAST_B>(A_ptr, B1_ptr, B2_ptr, SFA_ptr, SFB1_ptr, SFB2_ptr, C_ptr, M, N);

  if (false) {}
  else if (M == 256 && K == 7168) LAUNCH(7168,  64, 1)
  else if (M == 512 && K == 7168) LAUNCH(7168, 128, 2)
  else if (M == 256 && K == 4096) LAUNCH(4096,  64, 1)
  // the rest
  else if (K ==  256) LAUNCH( 256, 128, 1)
  else if (K ==  512) LAUNCH( 512, 128, 1)
  else if (K == 1536) LAUNCH(1536, 128, 1)
  else if (K == 2048) LAUNCH(2048, 128, 1)
  else if (K == 2304) LAUNCH(2304, 128, 1)
  else if (K == 7168) LAUNCH(7168, 128, 1)
