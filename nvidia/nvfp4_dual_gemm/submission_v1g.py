  ...
  if (warp_id == NUM_WARPS - 2) {
    ...
    if (elect_sync()) {
      ...
      for (int iter_k = 0; iter_k < num_iters; iter_k++) {
        ...
        tma_2d_gmem2smem(A_smem, &A_tmap, off_k, off_m, mbar_addr, cache_A);
        tma_2d_gmem2smem(B1_smem, &B1_tmap, off_k, off_n, mbar_addr, cache_B);
        tma_2d_gmem2smem(B2_smem, &B2_tmap, off_k, off_n, mbar_addr, cache_B);
...
void init_AB_tmap(
  ...
  uint64_t globalStrides[rank-1] = {global_width / 2};  // in bytes
...
#define LAUNCH(K_, BLOCK_N) \
  dual_gemm_launch<K_, BLOCK_N>(A_ptr, B1_ptr, B2_ptr, SFA_ptr, SFB1_ptr, SFB2_ptr, C_ptr, M, N);
