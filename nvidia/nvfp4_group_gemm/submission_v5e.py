...
__device__ __inline__
void stg_16(half *ptr, int t_row, int t_col) {
  asm volatile(
    "{\n"
    ...
    "st.relaxed.cta.global.L1::no_allocate%2.v8.b32 [%0], {b0, b1, b2, b3, b4, b5, b6, b7};\n"
    "}"
    :: "l"(ptr), "r"((t_row << 16) | t_col), "C"(l2_mod_ptx<l2_mod>::str)
  );
}
...
template <int NUM_GROUPS, int N, int K, int NUM_STAGES, int CTA_GROUP>
__global__
...
void kernel_cutlass(const __grid_constant__ Arguments<NUM_GROUPS> args, int K_dyn) {
  ...
  const int taddr_addr = epilogue_mbar_addr + 2 * 8;
  ...
  if (warp_id == NUM_WARPS - 2) {
    ...
    if (elect_sync()) {
      ...
      auto issue_tma = [&](int iter_k, int &stage_id, bool do_wait) {
        ...
        const int off_sfa = (bid_m * rest_k * 512 + iter_k * SF_size + cta_rank * (SF_size / 2)) / 8;
        ...
        if constexpr (CTA_GROUP == 2)
          tma_1d_g2s_mcast<CTA_GROUP>(SFA_smem, SFA_tmap, off_sfa, mbar_addr, 0b11, cache_A);
        else
          tma_1d_g2s<CTA_GROUP>(SFA_smem, SFA_tmap, off_sfa, mbar_addr, cache_A);
    ...
    for (; raw_bid_m < num_m_tiles - gridDim.y; raw_bid_m += gridDim.y) {
      ...
      for (int m = 0; m < BLOCK_M / WIDTH; m++) {
        ...
        if (col >= M) break;
        stg_16<L2_MOD::EVICT_LAST>(C_ptr + (row * stride_cn + col), cta_rank * BLOCK_N + warp_id * 32, stage_id * BLOCK_M + m * WIDTH);
      }
...

    {
      ...
      for (int m = 0; m < BLOCK_M / WIDTH; m++) {
        const int col = off_m + m * WIDTH;
        if (col >= M) break;
        stg_16<L2_MOD::NONE>(C_ptr + (row * stride_cn + col), cta_rank * BLOCK_N + warp_id * 32, stage_id * BLOCK_M + m * WIDTH);
      }
    }
...
void init_SF_tmap(CUtensorMap *tmap, void *ptr, uint64_t MN, uint64_t K, int CTA_GROUP) {
  ...
  uint32_t boxDim[rank]          = {SF_size / 8 / CTA_GROUP};
...
Arguments<NUM_GROUPS> create_args_template() {
  ...
  for (int i = 0; i < NUM_GROUPS; i++) {
    ...
    init_SF_tmap(args.SFA_tmap_list + i, nullptr, 1, K, CTA_GROUP);
    init_SF_tmap(args.SFB_tmap_list + i, nullptr, N, K, 1);
  }
  return args;
}
...
void group_gemm_launch(
  ...
  auto this_kernel = kernel_cutlass<NUM_GROUPS, N, K, NUM_STAGES, CTA_GROUP>;
...
#define LAUNCH(G_, N_, K_, CTA_GROUP) \
  else if (G == G_ && N == N_ && K == K_) { \
    group_gemm_launch<G_, N_, K_, CTA_GROUP>(A_list, B_list, SFA_list, SFB_list, C_list); \
  }

  if (false) {}
  LAUNCH(8, 4096, 7168, 2)
  LAUNCH(8, 7168, 2048, 2)
  LAUNCH(2, 3072, 4096, 2)
  LAUNCH(2, 4096, 1536, 2)
