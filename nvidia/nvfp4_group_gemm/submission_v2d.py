...
void kernel_cutlass(const __grid_constant__ Arguments<NUM_GROUPS> args, int K_dyn) {
  ...
  constexpr int B_size = (BLOCK_N / CTA_GROUP) * BLOCK_K / 2;  // each CTA holds half
  ...
  if (warp_id == 0 && elect_sync()) {
    ...
    for (int i = 0; i < NUM_STAGES; i++) {
      mbarrier_init(tma_mbar_addr + i * 8, CTA_GROUP);  // both CTAs report to CTA0
      mbarrier_init(mma_mbar_addr + i * 8, 1);
    }
    for (int i = 0; i < 2; i++) {
      mbarrier_init(mainloop_mbar_addr + i * 8, 1);
      mbarrier_init(epilogue_mbar_addr + i * 8, 4 * WARP_SIZE * CTA_GROUP);  // both CTAs report to CTA0
    }
  ...
  auto find_bid = [&](int bid, int& group_id, int& M, int& bid_m, int& bid_n) {
    ...
    if constexpr (raster == Raster::M) {
      const int grid_m = cdiv(M, BLOCK_M * CTA_GROUP) * CTA_GROUP;  // make sure grid_m % CTA_GROUP == 0
      bid_m = bid % grid_m;
      bid_n = bid / grid_m;
    }
  ...
  if (warp_id == NUM_WARPS - 2) {
    ...
    if (elect_sync()) {
      ...
      int group_id = 0;
      const int tma_mbar_addr_ = tma_mbar_addr & 0xFEFFFFFF;  // report to CTA0
      ...
      for (int bid = blockIdx.x; bid < num_tiles; bid += gridDim.x) {
        ...
        const int off_bn = off_n + cta_rank * (BLOCK_N / CTA_GROUP);  // select half of B
        ...
        if (off_m < M) {
          for (int iter_k = 0; iter_k < K_dyn / BLOCK_K; iter_k++) {
            ...
            // issue TMA
            tma_3d_g2s<CTA_GROUP>(B_smem, B_tmap, 0, off_bn, iter_k, mbar_addr, cache_B);
            ...
            // NOTE: 2D TMA for SF is slightly slower
            ...
            for (int i = 0; i < BLOCK_N / 128; i++)
              tma_1d_g2s<CTA_GROUP>(SFB_smem + i * 2048, SFB_tmap, (off_sfb + i * rest_k * 512) / 8, mbar_addr, cache_B);
          ...
          for (int iter_k = 0; iter_k < K_dyn / BLOCK_K; iter_k++) {
            ...
            mbarrier_arrive_expect_tx(mbar_addr, B_size);  // signal TMA done
    ...
    if (cta_rank == 0 && elect_sync()) {
      ...
      //   1st stage: [0, 128]
      //   2nd stage:         [128, 256]
      //   (unsued) :                   [256, 464]          [496, 512]
      //   SFA + SFB:                             [464, 496]
      //
      // for BLOCK_N = 256 (from https://www.youtube.com/watch?v=XzN8EtgEulU)
      //   1st stage: [0, 208][208, 256]
      //   2nd stage:         [208, 256][256, 464]
      //   SFA + SFB:                             [464, 512]
      //
      // signalling strategy
      //   MMA     : stage1 -> signal done                                   -> stage2
      //   epilogue:                       -> load 48 columns -> signal done

      for (int bid = blockIdx.x; bid < num_tiles; bid += gridDim.x) {
        const int acc_tmem = outer_stage * (BLOCK_N == 256 ? 208 : BLOCK_N);

        if constexpr (BLOCK_N == 256)
          mbarrier_wait(epilogue_mbar_addr + outer_stage * 8, 1);
        else
        ...
        for (int iter_k = 0; iter_k < K_dyn / BLOCK_K; iter_k++) {
          ...
          // use the last 48 columns for SFA and SFB, regardless of BLOCK_N
          int sfa_tmem = 464;
          ...
          for (int i = 0; i < BLOCK_N / 128; i++)
            tcgen05_cp_nvfp4<CTA_GROUP>(sfb_tmem + i * 4, sfb_desc + i * (2048 >> 4));
          tcgen05_mma_nvfp4<CTA_GROUP>(acc_tmem, a_desc, b_desc, i_desc, sfa_tmem, sfb_tmem, iter_k);

          for (int k = 1; k < BLOCK_K / MMA_K; k++) {
            // next 4 columns
            sfa_tmem += 4;
            sfb_tmem += 4 * (BLOCK_N / 128);
            ...
            for (int i = 0; i < BLOCK_N / 128; i++)
              tcgen05_cp_nvfp4<CTA_GROUP>(sfb_tmem + i * 4, sfb_desc + i * (2048 >> 4));
            tcgen05_mma_nvfp4<CTA_GROUP>(acc_tmem, a_desc, b_desc, i_desc, sfa_tmem, sfb_tmem, 1);
          }

          tcgen05_commit_mcast<CTA_GROUP>(mma_mbar_addr + inner_stage * 8, cta_mask);  // signal MMA done
          inner_stage = (inner_stage + 1) % NUM_STAGES;
          if (inner_stage == 0)
            tma_phase ^= 1;
        }

        tcgen05_commit_mcast<CTA_GROUP>(mainloop_mbar_addr + outer_stage * 8, cta_mask);  // signal mainloop done

        // wait for the completion of overlapping 48 columns
        if constexpr (BLOCK_N == 256)
          mbarrier_wait(epilogue_mbar_addr + outer_stage * 8, 0);
    ...
    int group_id = 0;
    const int epilogue_mbar_addr_ = epilogue_mbar_addr & 0xFEFFFFFF;  // report to CTA0
    ...
    for (int bid = blockIdx.x; bid < num_tiles; bid += gridDim.x) {
      ...
      const int t_row = cta_rank * BLOCK_M + warp_id * 32;

      if constexpr (BLOCK_N == 128) {
        for (int n = 0; n < BLOCK_N / WIDTH; n++) {
          float tmp[WIDTH];
          tcgen05_ld_32x32b<WIDTH>(tmp, t_row, stage_id * BLOCK_N + n * WIDTH);
      ...
      if constexpr (BLOCK_N == 256) {
        // epilogue for the overlapping tmem columns [208, 256]
        // stage_id | tmem columns | output columns
        // ---------|--------------|---------------
        // 0        | [208, 256]   | [208, 256]
        // 1        | [208, 256]   | [  0,  48]
        for (int n = 0; n < 3; n++) {
          float tmp[WIDTH];
          tcgen05_ld_32x32b<WIDTH>(tmp, t_row, 208 + n * WIDTH);
          asm volatile("tcgen05.wait::ld.sync.aligned;");

          const int row = off_m + tid;
          const int col = off_n + 208 * (1 - stage_id) + n * WIDTH;
          if (row < M)
            stg_16(C_ptr + (row * N + col), tmp);
        }

        // signal done 48 columns
        ...
        for (int n = 0; n < 13; n++) {
          float tmp[WIDTH];
          tcgen05_ld_32x32b<WIDTH>(tmp, t_row, stage_id * 256 + n * WIDTH);
          asm volatile("tcgen05.wait::ld.sync.aligned;");

          const int row = off_m + tid;
          const int col = off_n + 48 * stage_id + n * WIDTH;
...
void init_SF_tmap(CUtensorMap *tmap, void *ptr, uint64_t global_size, uint32_t shared_size) {
...
template <int NUM_GROUPS, int BLOCK_N, int N, int K, int CTA_GROUP, int NUM_BLOCKS, Raster raster>
void group_gemm_launch(
  ...
  for (int i = 0; i < NUM_GROUPS; i++) {
    ...
    init_AB_tmap(args.B_tmap_list + i, B_list[i].data_ptr(), N, K, BLOCK_N / CTA_GROUP, BLOCK_K);  // each TMA loads half of B
    init_SF_tmap(args.SFA_tmap_list + i, SFA_list[i].data_ptr(), cdiv(M, 128) * 128 * K / 16, 128 * BLOCK_K / 16);
    init_SF_tmap(args.SFB_tmap_list + i, SFB_list[i].data_ptr(), N * K / 16, 128 * BLOCK_K / 16);
    args.C_ptr_list[i] = reinterpret_cast<half *>(C_list[i].data_ptr());
    args.M_list[i] = M;
    args.grid_cu[i + 1] = args.grid_cu[i] + grid_m * grid_n;
  }

  // using only 128 SMs is faster than 148 SMs for benchmark.0.
  // likely voodoo cache behavior.
  const int grid = std::min(NUM_BLOCKS, args.grid_cu[NUM_GROUPS]);
  ...
  auto this_kernel = kernel_cutlass<NUM_GROUPS, BLOCK_N, N, K, NUM_STAGES, CTA_GROUP, raster>;
...
#define LAUNCH(G_, BN, N_, K_, CTA_GROUP, NUM_BLOCKS, raster) \
  else if (G == G_ && N == N_ && K == K_) { \
    group_gemm_launch<G_, BN, N_, K_, CTA_GROUP, NUM_BLOCKS, raster>(A_list, B_list, SFA_list, SFB_list, C_list); \
  }

  // don't know why it only works for benchmark.2. but it's not faster anyway...
  if (false) {}
  LAUNCH(8, 128, 4096, 7168, 1, 128, Raster::N)  // not sure why this is slower with Raster::M
  LAUNCH(8, 256, 7168, 2048, 1, 128, Raster::M)  // faster with Raster::M
  LAUNCH(2, 128, 3072, 4096, 2, 148, Raster::M)  // Raster::M and Raster::N have the same speed
  LAUNCH(2, 128, 4096, 1536, 1, 128, Raster::M)
