...
template <int NUM_GROUPS, int N, int K, int NUM_STAGES, int CTA_GROUP, Raster raster, bool DO_PROFILE>
__global__
__cluster_dims__(2, 1, 1)
__launch_bounds__(TB_SIZE)
void kernel_cutlass(
  const __grid_constant__ Arguments<NUM_GROUPS> args,
  int K_dyn,
  ...
  if (warp_id == NUM_WARPS - 2) {
    ...
    if (elect_sync()) {
      ...
      for (int bid = blockIdx.x; bid < num_tiles; bid += gridDim.x) {
        ...
        for (int iter_k = 0; iter_k < K_dyn / BLOCK_K; iter_k++) {
          ...
          profiler.stamp(ProfilerTag::WAIT_MMA);
          ...
          if (stage_id == 0)
            mma_phase ^= 1;
          profiler.stamp(ProfilerTag::ISSUE_TMA);
        }
      }
    }
  }
    ...
    if (cta_rank == 0 && elect_sync()) {
      ...
      for (int bid = blockIdx.x; bid < num_tiles; bid += gridDim.x) {
        ...
        profiler.stamp(ProfilerTag::WAIT_EPILOGUE);
        ...
        for (int iter_k = 0; iter_k < K_dyn / BLOCK_K; iter_k++) {
          ...
          profiler.stamp(ProfilerTag::WAIT_TMA);
          ...
          if (inner_stage == 0)
            tma_phase ^= 1;
          profiler.stamp(ProfilerTag::ISSUE_MMA);
        }
    ...
    if (elect_sync()) profiler.stamp(ProfilerTag::SETUP);

    for (int bid = blockIdx.x; bid < num_tiles; bid += gridDim.x) {
      int M, bid_m, bid_n;
      find_bid(bid, group_id, M, bid_m, bid_n);
      if (elect_sync()) profiler.stamp(ProfilerTag::COMPUTE_BID);
      ...
      if (elect_sync()) profiler.stamp(ProfilerTag::WAIT_MAINLOOP);
      ...
      for (int m = 0; m < BLOCK_M / WIDTH; m++) {
        ...
        if (elect_sync()) profiler.stamp(ProfilerTag::EPILOGUE_TMEM_LOAD);

        const int row = off_n + tid;
        const int col = off_m + m * WIDTH;
        if (col < M)
          stg_16(C_ptr + (row * stride_cn + col), tmp);
        if (elect_sync()) profiler.stamp(ProfilerTag::EPILOGUE_GMEM_STORE);
      }
    ...
    if (elect_sync()) profiler.stamp(ProfilerTag::EPILOGUE_SYNC);
...
void group_gemm_launch(
  ...
  if (profiler_ptr == nullptr) {
    auto this_kernel = kernel_cutlass<NUM_GROUPS, N, K, NUM_STAGES, CTA_GROUP, raster, false>;
    cudaFuncSetAttribute(this_kernel, cudaFuncAttributeMaxDynamicSharedMemorySize, smem_size);
    this_kernel<<<grid, TB_SIZE, smem_size>>>(args, K, profiler_ptr, num_entries);
  }
  else {
    auto this_kernel = kernel_cutlass<NUM_GROUPS, N, K, NUM_STAGES, CTA_GROUP, raster, true>;
    cudaFuncSetAttribute(this_kernel, cudaFuncAttributeMaxDynamicSharedMemorySize, smem_size);
    this_kernel<<<grid, TB_SIZE, smem_size>>>(args, K, profiler_ptr, num_entries);
  }
}
...
void group_gemm(
  ...
  at::TensorList SFB_list,
  at::TensorList C_list,
  c10::optional<at::Tensor> profiler
) {
  int64_t *profiler_ptr = nullptr;
  int num_entries = 0;
  if (profiler.has_value() && profiler.value().defined()) {
    num_entries = (profiler->size(2) - 1) / 2;
    profiler_ptr = profiler->data_ptr<int64_t>();
  }
...
#define LAUNCH(G_, N_, K_, CTA_GROUP, raster) \
    ...
    group_gemm_launch<G_, N_, K_, CTA_GROUP, raster>(A_list, B_list, SFA_list, SFB_list, C_list, profiler_ptr, num_entries); \
  }
...
TORCH_LIBRARY(my_module, m) {
  m.def("group_gemm(Tensor[] A_list, Tensor[] B_list, Tensor[] SFA_list, Tensor[] SFB_list, Tensor(a!)[] C_list, Tensor? profiler=None) -> ()");
...
def custom_kernel(data: input_t) -> output_t:
    ...
    for _, N, K, _ in shape_list:
        ...
        G = len(shape_list)
        path = Path(f"profile_data/trace_{G=}_{N=}_{K=}.json.gz")

        if not path.exists():
            num_entries = 1000
            profiler = torch.zeros(148, 6, 1 + num_entries * 2, dtype=torch.int64, device="cuda")
            group_gemm(A_list, B_list, SFA_list, SFB_list, C_list, profiler)
            torch.cuda.synchronize()

            events = []
            profiler_data = profiler.tolist()
            for sm_id in range(148):
                for warp_id in range(6):
                    ...
                    for i in range(1, cnt):
                        ...
                        events.append(dict(name=TAGS[tag], ph="X", ts=start, dur=ts - start, pid=sm_id, tid=sm_id + warp_id))
                        start = ts
