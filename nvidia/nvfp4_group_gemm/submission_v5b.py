...
template <int NUM_GROUPS, int BLOCK_N, int N, int K, int NUM_STAGES, int CTA_GROUP, Raster raster>
__global__
...
void kernel_cutlass(const __grid_constant__ Arguments<NUM_GROUPS> args, int K_dyn) {
  ...
  if (warp_id == NUM_WARPS - 2) {
    ...
    // MMA warp
    tcgen05_alloc<CTA_GROUP>(epilogue_mbar_addr + (BLOCK_N / 128) * 2 * 8, 512);  // allocate tmem

    // instruction desc
    constexpr uint32_t MMA_M = 128 * CTA_GROUP;
    ...
    if (cta_rank == 0 && elect_sync()) {
      ...
      // note that on each threadblock, MMA_M is limited to 128.
      // hence, to use BLOCK_N=256, we need to issue 2 MMA
      //   1st stage: [0, 128][128, 208][208, 256]
      //   2nd stage:                   [208, 256][256, 336][336, 464]
      //   SFA + SFB:                                                 [464, 512]
      //
      // signalling strategy
      ...
      for (int bid = blockIdx.x; bid < num_tiles; bid += gridDim.x) {
        const int acc0_tmem = outer_stage * (BLOCK_N == 256 ? 208 : BLOCK_M);
        ...
        for (int iter_k = 0; iter_k < K_dyn / BLOCK_K; iter_k++) {
          ...
          const int B1_smem = B0_smem + (B_size / 2);  // only used when BLOCK_N=256
          const int SFA_smem = B1_smem + (B_size / 2);
          const int SFB0_smem = SFA_smem + SF_size;
          const int SFB1_smem = SFB0_smem + SF_size;  // only used when BLOCK_N=256
          ...
          tcgen05_cp_nvfp4<CTA_GROUP>(sfb0_tmem, sfb0_desc);
          if constexpr (BLOCK_N == 256)
            tcgen05_cp_nvfp4<CTA_GROUP>(sfb1_tmem, sfb1_desc);
          tcgen05_mma_nvfp4<CTA_GROUP>(acc0_tmem, b0_desc, a_desc, i_desc, sfb0_tmem, sfa_tmem, iter_k);
          if constexpr (BLOCK_N == 256)
            tcgen05_mma_nvfp4<CTA_GROUP>(acc1_tmem, b1_desc, a_desc, i_desc, sfb1_tmem, sfa_tmem, iter_k);
          ...
          for (int k = 1; k < BLOCK_K / MMA_K; k++) {
            ...
            if constexpr (BLOCK_N == 256)
              tcgen05_cp_nvfp4<CTA_GROUP>(sfb1_tmem, sfb1_desc);
            tcgen05_mma_nvfp4<CTA_GROUP>(acc0_tmem, b0_desc, a_desc, i_desc, sfb0_tmem, sfa_tmem, 1);
            if constexpr (BLOCK_N == 256)
              tcgen05_mma_nvfp4<CTA_GROUP>(acc1_tmem, b1_desc, a_desc, i_desc, sfb1_tmem, sfa_tmem, 1);
          }
        ...
        if constexpr (BLOCK_N == 256)
          mbarrier_wait(epilogue_mbar_addr + (2 + outer_stage) * 8, epilogue_phase ^ 1);
    ...
    for (int bid = blockIdx.x; bid < num_tiles; bid += gridDim.x) {
      ...
      if constexpr (BLOCK_N == 256) {
        ...
        // stage_id | tmem columns | MMA subtile | output columns
        // ---------|--------------|-------------|----------------
        // 0        | [208, 256]   | 1           | [80, 128]
        // 1        | [208, 256]   | 0           | [ 0,  48]
        for (int m = 0; m < 3; m++) {
          float tmp[WIDTH];
          tcgen05_ld_32x32b<WIDTH>(tmp, cta_rank * 128 + warp_id * 32, 208 + m * WIDTH);
          asm volatile("tcgen05.wait::ld.sync.aligned;");

          const int row = off_n + 128 * (1 - stage_id) + tid;
          const int col = off_m + 80 * (1 - stage_id) + m * WIDTH;
          if (col < M)
            stg_16(C_ptr + (row * stride_cn + col), tmp);
        }

        // signal done 48 columns
        mbarrier_arrive(epilogue_mbar_addr_ + (2 + stage_id) * 8);
        ...
        for (int m = 0; m < 5; m++) {
          float tmp[WIDTH];
          tcgen05_ld_32x32b<WIDTH>(tmp, cta_rank * 128 + warp_id * 32, 128 * (stage_id + 1) + m * WIDTH);
          asm volatile("tcgen05.wait::ld.sync.aligned;");

          const int row = off_n + 128 * (1 - stage_id) + tid;
          const int col = off_m + 48 * stage_id + m * WIDTH;
          if (col < M)
            stg_16(C_ptr + (row * stride_cn + col), tmp);
        }
        ...
        for (int m = 0; m < 8; m++) {
          float tmp[WIDTH];
          tcgen05_ld_32x32b<WIDTH>(tmp, cta_rank * 128 + warp_id * 32, 336 * stage_id + m * WIDTH);
          asm volatile("tcgen05.wait::ld.sync.aligned;");

          const int row = off_n + 128 * stage_id + tid;
          const int col = off_m + m * WIDTH;
          if (col < M)
            stg_16(C_ptr + (row * stride_cn + col), tmp);
        }
...
template <int NUM_GROUPS, int BLOCK_N, int N, int K, int CTA_GROUP>
...
template <int NUM_GROUPS, int BLOCK_N, int N, int K, int CTA_GROUP, Raster raster>
void group_gemm_launch(
  ...
  static Arguments<NUM_GROUPS> args = create_args_template<NUM_GROUPS, BLOCK_N, N, K, CTA_GROUP>();
  ...
  constexpr int static_size = (1 + BLOCK_N / 128) * 2 * 8 + 4;  // 2 mainloop_mbar, (BLOCK_N/128)*2 epilogue_mbar, tmem_addr
...
#define LAUNCH(G_, BN, N_, K_, CTA_GROUP, raster) \
  else if (G == G_ && N == N_ && K == K_) { \
    group_gemm_launch<G_, BN, N_, K_, CTA_GROUP, raster>(A_list, B_list, SFA_list, SFB_list, C_list); \
  }

  if (false) {}
  LAUNCH(8, 128, 4096, 7168, 2, Raster::N)
  LAUNCH(8, 256, 7168, 2048, 2, Raster::N)
  LAUNCH(2, 128, 3072, 4096, 2, Raster::N)
  LAUNCH(2, 128, 4096, 1536, 1, Raster::N)
