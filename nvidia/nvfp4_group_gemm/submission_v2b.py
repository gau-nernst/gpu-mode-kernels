...
template <int NUM_GROUPS, int BLOCK_N, int N, int K, int NUM_STAGES>
__global__
    ...
    if (elect_sync()) {
      ...
      for (int bid = blockIdx.x; bid < num_tiles; bid += gridDim.x) {
        ...
        for (int iter_k = 0; iter_k < K / BLOCK_K; iter_k++) {
          ...
          const char *SFB_src = SFB_ptr + (bid_n * (BLOCK_N / 128) * rest_k * 512 + iter_k * 2048);
          const char *SFA_src = SFA_ptr + (bid_m * rest_k * 512 + iter_k * 2048);
          for (int i = 0; i < BLOCK_N / 128; i++)
            tma_gmem2smem(SFB_smem + i * 2048, SFB_src + (i * rest_k * 512), SF_size, mbar_addr, cache_B);
...
void group_gemm_launch(
  ...
  auto this_kernel = kernel_cutlass<NUM_GROUPS, BLOCK_N, N, K, NUM_STAGES>;
...
#define LAUNCH(G_, BN, N_, K_) \
  else if (G == G_ && N == N_ && K == K_) { \
    group_gemm_launch<G_, BN, N_, K_>(A_list, B_list, SFA_list, SFB_list, C_list); \
  }

  if (false) {}
  LAUNCH(8, 128, 4096, 7168)
  LAUNCH(8, 256, 7168, 2048)
  LAUNCH(2, 128, 3072, 4096)
  LAUNCH(2, 128, 4096, 1536)
