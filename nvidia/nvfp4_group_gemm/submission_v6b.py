  ...
  if (warp_id == NUM_WARPS - 2) {
    ...
    if (cta_rank == 0 && elect_sync()) {
      ...
      for (int raw_bid_m = blockIdx.y; raw_bid_m < num_m_tiles; raw_bid_m += gridDim.y) {
        ...
        const int acc0_tmem = outer_stage * 128;
        const int acc1_tmem = acc0_tmem + 128;
        ...
        for (int iter_k = 0; iter_k < K_dyn / BLOCK_K; iter_k++) {
          ...
          tcgen05_mma_nvfp4<CTA_GROUP, COLLECTOR_USAGE::A_FILL>(acc0_tmem, b_desc, a0_desc, i_desc, sfb_tmem, sfa0_tmem, iter_k);
          if (BLOCK_M == 256 && do_2nd_mma) {
            tcgen05_cp_nvfp4<CTA_GROUP>(sfa1_tmem, sfa1_desc);
            tcgen05_mma_nvfp4<CTA_GROUP, COLLECTOR_USAGE::A_LASTUSE>(acc1_tmem, b_desc, a1_desc, i_desc, sfb_tmem, sfa1_tmem, iter_k);
          }
          ...
          for (int k = 1; k < BLOCK_K / MMA_K; k++) {
            ...
            tcgen05_cp_nvfp4<CTA_GROUP>(sfb_tmem, sfb_desc);
            tcgen05_cp_nvfp4<CTA_GROUP>(sfa0_tmem, sfa0_desc);
            tcgen05_mma_nvfp4<CTA_GROUP, COLLECTOR_USAGE::A_FILL>(acc0_tmem, b_desc, a0_desc, i_desc, sfb_tmem, sfa0_tmem, 1);
            if (BLOCK_M == 256 && do_2nd_mma) {
              tcgen05_cp_nvfp4<CTA_GROUP>(sfa1_tmem, sfa1_desc);
              tcgen05_mma_nvfp4<CTA_GROUP, COLLECTOR_USAGE::A_LASTUSE>(acc1_tmem, b_desc, a1_desc, i_desc, sfb_tmem, sfa1_tmem, 1);
            }
          }
