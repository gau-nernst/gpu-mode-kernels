...
template <int NUM_GROUPS, int BLOCK_N, int N, int K, int NUM_STAGES, Raster raster>
__global__
...
void kernel_cutlass(const __grid_constant__ Arguments<NUM_GROUPS> args, int K_dyn) {
  ...
  if (warp_id == NUM_WARPS - 2) {
    ...
    // MMA warp
    tcgen05_alloc(epilogue_mbar_addr + (BLOCK_N / 128) * 2 * 8, 512);  // allocate tmem
    ...
    if (elect_sync()) {
      ...
      for (int bid = blockIdx.x; bid < num_tiles; bid += gridDim.x) {
        ...
        for (int iter_k = 0; iter_k < K_dyn / BLOCK_K; iter_k++) {
          ...
          for (int i = 0; i < BLOCK_N / 128; i++)
            tcgen05_cp_nvfp4(sfb_tmem + i * 4, sfb_desc + i * (2048 >> 4));
    ...
    for (int bid = blockIdx.x; bid < num_tiles; bid += gridDim.x) {
      ...
      half *C_ptr = args.C_ptr_list[group_id] + (row * N + off_n);

      //const int warp_partial = off_m + warp_id * 32 < M;  // this is a bit slower
      const int warp_partial = 1;
      ...
      if constexpr (BLOCK_N == 256) {
        ...
        if (warp_partial) {
          for (int n = 0; n < 3; n++) {
            float tmp[WIDTH];
            tcgen05_ld_32x32b<WIDTH>(tmp, warp_id * 32, 208 + n * WIDTH);
            asm volatile("tcgen05.wait::ld.sync.aligned;");
            if (row < M)
              stg_16(C_ptr + (208 * (1 - stage_id) + n * WIDTH), tmp);
          }
        }

        // signal done 48 columns
        mbarrier_arrive(epilogue_mbar_addr + (2 + stage_id) * 8);
        ...
        if (warp_partial) {
          for (int n = 0; n < 13; n++) {
            float tmp[WIDTH];
            tcgen05_ld_32x32b<WIDTH>(tmp, warp_id * 32, stage_id * 256 + n * WIDTH);
            asm volatile("tcgen05.wait::ld.sync.aligned;");
            if (row < M)
              stg_16(C_ptr + (48 * stage_id + n * WIDTH), tmp);
          }
        }
      }
...
template <int NUM_GROUPS, int BLOCK_N, int N, int K>
...
template <int NUM_GROUPS, int BLOCK_N, int N, int K, Raster raster>
void group_gemm_launch(
  ...
  static Arguments<NUM_GROUPS> args = create_args_template<NUM_GROUPS, BLOCK_N, N, K>();
  ...
  const int grid = std::min(128, args.grid_cu[NUM_GROUPS]);
  ...
  auto this_kernel = kernel_cutlass<NUM_GROUPS, BLOCK_N, N, K, NUM_STAGES, raster>;
...
#define LAUNCH(G_, BN, N_, K_, raster) \
  else if (G == G_ && N == N_ && K == K_) { \
    group_gemm_launch<G_, BN, N_, K_, raster>(A_list, B_list, SFA_list, SFB_list, C_list); \
  }

  if (false) {}
  LAUNCH(8, 128, 4096, 7168, Raster::N)  // not sure why this is slower with Raster::M
  LAUNCH(8, 256, 7168, 2048, Raster::M)  // faster with Raster::M
  LAUNCH(2, 128, 3072, 4096, Raster::N)  // Raster::M and Raster::N have the same speed
  LAUNCH(2, 128, 4096, 1536, Raster::N)
