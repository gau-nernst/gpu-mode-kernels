...
struct Arguments {
  ...
  CUtensorMap C_tmap_list[NUM_GROUPS];
...
void kernel_cutlass(const __grid_constant__ Arguments<NUM_GROUPS> args, int K_dyn) {
  ...
  int group_id = 0;
  int M, bid_m, bid_n;
  find_bid(blockIdx.x, group_id, M, bid_m, bid_n);
  ...
  if (warp_id == 0 && elect_sync()) {
    ...
    prefetch_tensormap(args.C_tmap_list + group_id);
  }
  ...
  if (warp_id == NUM_WARPS - 2) {
    ...
    auto sts_8 = [](int addr, float *tmp) {
      asm volatile(
        ...
        "stmatrix.sync.aligned.m8n8.x4.shared::cta.b16 [%0], {out0, out1, out2, out3};\n"
        "}"
        :: "r"(addr),
        "f"(tmp[0]), "f"(tmp[1]), "f"(tmp[2]), "f"(tmp[3]),
    ...
    auto C_tmap = args.C_tmap_list + group_id;

    // pre-compute stmatrix address, including swizzling
    int smem_st = smem + warp_id * 32 * BLOCK_N * sizeof(half);  // each warp "owns" fp16[32][BLOCK_N] of smem
    {
      const int row = lane_id % 16;
      const int col = (lane_id / 16) ^ (lane_id % 8);
      smem_st += row * 128 + col * 16;
    }
    ...
    for (int i = 0; i < 2; i++) {
      const int row = warp_id * 32 + i * 16;

      float tmp[BLOCK_N / 2];
      tcgen05_ld_16x256b<BLOCK_N / 8>(tmp, row, 0);
      asm volatile("tcgen05.wait::ld.sync.aligned;");

      // each j iteration consumes fp16[16][64] tile
      // each n iteration consumes fp16[16][16] tile
      for (int j = 0; j < BLOCK_N / 64; j++) {
        for (int n = 0; n < 64 / 16; n++)
          sts_8(smem_st ^ (n * 32), tmp + j * 32 + n * 8);
        smem_st += 16 * 128;  // in bytes
      }

      // make smem stores visible to async proxy
      asm volatile("fence.proxy.async::generic.release.sync_restrict::shared::cta.cluster;");

      if (elect_sync()) {
        // TMA issue observes smem stores
        asm volatile("fence.proxy.async::generic.acquire.sync_restrict::shared::cluster.cluster;");
        tma_3d_s2g(C_tmap, 0, off_m + row, off_n / 64, smem + row * BLOCK_N * sizeof(half));
...
void group_gemm_launch(
  ...
  for (int i = 0; i < NUM_GROUPS; i++) {
    ...
    {
      constexpr uint32_t rank = 3;
      uint64_t globalDim[rank]       = {64, M, N / 64};
      uint64_t globalStrides[rank-1] = {N * sizeof(half), 128};  // in bytes
      uint32_t boxDim[rank]          = {64, 16, BLOCK_N / 64};
      uint32_t elementStrides[rank]  = {1, 1, 1};
