...
void group_gemm_launch(
  ...
  for (int i = 0; i < NUM_GROUPS; i++) {
    ...
    //cuTensorMapReplaceAddress(args.B_tmap_list + i, (void *)(B_list[i].data_ptr()));

    // exploit the internal encodings of CUtensorMap. doesn't seem to be faster.
    reinterpret_cast<void **>(args.A_tmap_list + i)[0] = A_list[i].data_ptr();
    reinterpret_cast<int *>(args.A_tmap_list + i)[9] = M - 1;
    reinterpret_cast<void **>(args.B_tmap_list + i)[0] = B_list[i].data_ptr();
