  ...
  if (warp_id == NUM_WARPS - 2) {
    ...
    auto sts_8 = [](int addr, float *tmp) {
      asm volatile(
        ...
        "st.shared.v4.b32 [%0], {out0, out1, out2, out3};\n"
        "}"
        :: "r"(addr),
        "f"(tmp[0]), "f"(tmp[1]), "f"(tmp[2]), "f"(tmp[3]),
    ...
    int smem_st = smem + tid * 128 + (tid % 8) * 16;
    ...
    // TODO: 16x128b, stmatrix
    // TODO: split TMA into 2 tiles? so that we can issue one earlier
    for (int i = 0; i < BLOCK_N / 64; i++) {
      float tmp[64];
      tcgen05_ld_32x32b<64>(tmp, warp_id * 32, i * 64);
      asm volatile("tcgen05.wait::ld.sync.aligned;");

      for (int n = 0; n < 64 / WIDTH; n++)
        sts_8(smem_st ^ (n * 16), tmp + n * WIDTH);
      smem_st += BLOCK_M * 128;
...
void group_gemm_launch(
  ...
  for (int i = 0; i < NUM_GROUPS; i++) {
...

    {
      ...
      uint64_t globalStrides[rank-1] = {N * sizeof(half)};  // in bytes
