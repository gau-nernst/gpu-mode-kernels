...
__device__ inline
void clc_try_cancel(int addr, int mbar_addr) {
  asm volatile("clusterlaunchcontrol.try_cancel.async.shared::cta.mbarrier::complete_tx::bytes.b128 [%0], [%1];"
              :: "r"(addr), "r"(mbar_addr));
}

__device__ inline
void clc_try_cancel_mcast(int addr, int mbar_addr) {
  asm volatile("clusterlaunchcontrol.try_cancel.async.shared::cta.mbarrier::complete_tx::bytes.multicast::cluster::all.b128 [%0], [%1];"
              :: "r"(addr), "r"(mbar_addr));
}

// https://github.com/NVIDIA/cutlass/blob/8dbce014/include/cutlass/gemm/kernel/sm100_tile_scheduler.hpp#L410
__device__ inline
uint32_t clc_query_cancel(int *bid, int addr) {
  uint32_t pred = 0;
  asm volatile(
    "{\n\t"
    ".reg .pred p1;\n\t"
...
void kernel_cutlass(const __grid_constant__ Arguments<NUM_GROUPS> args, int K_dyn) {
  ...
  const int clc_addr = epilogue_mbar_addr + 2 * 8;
  const int clc_mbar_addr = clc_addr + 16;
  const int taddr_addr = clc_mbar_addr + 2 * 8;
  ...
  if (warp_id == 0 && elect_sync()) {
    ...
    mbarrier_init(clc_mbar_addr, 1);  // producer
    mbarrier_init(clc_mbar_addr + 8, 1 + (1 + 4 * WARP_SIZE) * CTA_GROUP);  // consumer
  ...
  if (warp_id == 4) {
    ...
    if (elect_sync()) {
      ...
      // TODO: 2-stage this, so that consumer doesn't block clc try_cancel

      if (cta_rank == 0) {
        // issue try-cancel on CTA0
        while (true) {
          if constexpr (CTA_GROUP == 2)
            clc_try_cancel_mcast(clc_addr, clc_mbar_addr);
          else
            clc_try_cancel(clc_addr, clc_mbar_addr);
          mbarrier_arrive_expect_tx(clc_mbar_addr, 16);

          // wait for try-cancel to finish, then query the result
          // if return false -> break
          int bid;
          mbarrier_wait(clc_mbar_addr, phase);
          if (!clc_query_cancel(&bid, clc_addr)) break;

          // wait for consumers to finish
          mbarrier_wait(clc_mbar_addr + 8, phase);
          phase ^= 1;
        }
      }
      else {
        // on CTA1, just arrive on mbarrier
        while (true) {
          mbarrier_arrive_expect_tx(clc_mbar_addr, 16);
    ...
    if (elect_sync()) {
      ...
      const int clc_mbar_addr_ = CTA_GROUP == 2 ? (clc_mbar_addr & 0xFEFFFFFF) : clc_mbar_addr;

      while (true) {
        int group_id, M, bid_m, bid_n;
        ...
        // get the next block
        mbarrier_wait(clc_mbar_addr, clc_phase);
        if (!clc_query_cancel(&bid, clc_addr)) break;
        mbarrier_arrive(clc_mbar_addr_ + 8);

        // CLC returns CTA ID of the 1st CTA. adds CTA rank offset here.
        bid += cta_rank;
        clc_phase ^= 1;
      }
    }
    ...
    if (cta_rank == 0 && elect_sync()) {
      ...
      while (true) {
        ...
        // check if we need to stop (MMA warp doesn't care about block id)
        int bid;  // throw away
        mbarrier_wait(clc_mbar_addr, clc_phase);
        if (!clc_query_cancel(&bid, clc_addr)) break;
        mbarrier_arrive(clc_mbar_addr + 8);
        clc_phase ^= 1;
      }
    }
  }
...
void group_gemm_launch(
  ...
  const int grid = args.grid_cu[NUM_GROUPS];
  ...
  constexpr int static_size = 2 * 2 * 8   // 2 mainloop_mbar, 2 epilogue_mbar
                            + 16 + 2 * 8  // CLC: int4 result, and 2 mbar
                            + 4;          // tmem_addr
