...
template <int NUM_GROUPS, int BLOCK_K, int N, int K, int NUM_STAGES, int CTA_GROUP, L2_MOD epi_l2_mod>
__global__
...
void kernel_cutlass(const __grid_constant__ Arguments<NUM_GROUPS> args, int K_dyn) {
  ...
  if (warp_id == NUM_WARPS - 2) {
    ...
    if (elect_sync()) {
...

      {
        ...
        for (int iter_k = 0; iter_k < std::min(NUM_STAGES, K / BLOCK_K); iter_k++) {
          ...
          const int mbar_addr = tma_mbar_addr_ + iter_k * 8;
          const int B_smem = smem + iter_k * STAGE_SIZE;
          const int A_smem = B_smem + B_size;
          const int SFB_smem = A_smem + A_size;
          const int SFA_smem = SFB_smem + SF_size + cta_rank * (SF_size / 2);

          const int off_sfb = (bid_n * rest_k * 512 + iter_k * SF_size) / 1024;
          const int off_sfa = (bid_m * rest_k * 512 + iter_k * SF_size + cta_rank * (SF_size / 2)) / 1024;

          // issue MMA
          tma_3d_g2s<CTA_GROUP>(B_smem, B_tmap, 0, off_n, iter_k * (BLOCK_K / 256), mbar_addr, cache_B);
          tma_3d_g2s<CTA_GROUP>(A_smem, A_tmap, 0, off_m, iter_k * (BLOCK_K / 256), mbar_addr, cache_A);

          tma_2d_g2s<CTA_GROUP>(SFB_smem, SFB_tmap, 0, off_sfb, mbar_addr, cache_B);
          if constexpr (CTA_GROUP == 2)
            tma_2d_g2s_mcast<CTA_GROUP>(SFA_smem, SFA_tmap, 0, off_sfa, mbar_addr, 0b11, cache_A);
          else
            tma_2d_g2s<CTA_GROUP>(SFA_smem, SFA_tmap, 0, off_sfa, mbar_addr, cache_A);
    ...
    if (cta_rank == 0 && elect_sync()) {
      ...
      for (int bid_m = blockIdx.y; bid_m < num_m_tiles; bid_m += gridDim.y) {
        ...
        for (int iter_k = 0; iter_k < K_dyn / BLOCK_K; iter_k++) {
          ...
          const int A_smem = B_smem + B_size;
          const int SFB_smem = A_smem + A_size;
          const int SFA_smem = SFB_smem + SF_size;
          ...
          int sfb_tmem = BLOCK_N * 2;
          int sfa_tmem = sfb_tmem + 16 * (BLOCK_K / 256);

          // wait TMA
          mbarrier_wait(tma_mbar_addr + inner_stage * 8, tma_phase);

          int enable_input_d = iter_k;

          for (int k1 = 0; k1 < BLOCK_K / 256; k1++) {
            for (int k2 = 0; k2 < 256 / MMA_K; k2++) {
              tcgen05_cp_nvfp4<CTA_GROUP>(sfb_tmem, sfb_desc);
              tcgen05_cp_nvfp4<CTA_GROUP>(sfa_tmem, sfa_desc);
              tcgen05_mma_nvfp4<CTA_GROUP>(acc_tmem, b_desc, a_desc, i_desc, sfb_tmem, sfa_tmem, enable_input_d);
              enable_input_d = 1;
            ...
            // next (BLOCK_MN, 128B) tile
            b_desc += (BLOCK_N * 128 - 32 * (256 / MMA_K)) >> 4;
            a_desc += ((BLOCK_M / CTA_GROUP) * 128 - 32 * (256 / MMA_K)) >> 4;
          }
    ...
    auto stg_16 = [](half *ptr, float *tmp) {
      asm volatile(
        ...
        : "l"(ptr), "C"(l2_mod_ptx<epi_l2_mod>::str)
      );
    };
    ...
    for (; raw_bid_m < num_m_tiles - gridDim.y; raw_bid_m += gridDim.y) {
      ...
      tcgen05_ld_32x32b<BLOCK_M>(tmp, cta_rank * BLOCK_N + warp_id * 32, stage_id * BLOCK_M);
      asm volatile("tcgen05.wait::ld.sync.aligned;");

      for (int m = 0; m < BLOCK_M / 16; m++) {
        const int col = off_m + m * 16;
        if (col >= M) break;
        stg_16(C_ptr + (row * stride_cn + col), tmp + m * 16);
      }
...
void init_SF_tmap(CUtensorMap *tmap, void *ptr, uint64_t MN, uint64_t K, uint32_t BLOCK_K, int CTA_GROUP) {
  MN = cdiv(MN, 128) * 128;  // round up to multiple of 128

  const uint64_t global_size = MN * K / 16;
  const int SF_size = 128 * BLOCK_K / 16;

  // use int64 as dtype, hence divide sizes by 8
  constexpr uint32_t rank = 2;
  uint64_t globalDim[rank]       = {1024 / 8, global_size / 1024};
  uint64_t globalStrides[rank-1] = {1024};  // in bytes
  uint32_t boxDim[rank]          = {1024 / 8, SF_size / 1024 / CTA_GROUP};
...
template <int NUM_GROUPS, int BLOCK_K, int N, int K, int CTA_GROUP>
Arguments<NUM_GROUPS> create_args_template() {
  ...
  for (int i = 0; i < NUM_GROUPS; i++) {
    ...
    init_SF_tmap(args.SFA_tmap_list + i, nullptr, 1, K, BLOCK_K, CTA_GROUP);
    init_SF_tmap(args.SFB_tmap_list + i, nullptr, N, K, BLOCK_K, 1);
  }
  return args;
}
...
template <int NUM_GROUPS, int BLOCK_K, int N, int K, int CTA_GROUP, L2_MOD epi_l2_mod>
void group_gemm_launch(
  ...
  static Arguments<NUM_GROUPS> args = create_args_template<NUM_GROUPS, BLOCK_K, N, K, CTA_GROUP>();
  ...
  for (int i = 0; i < NUM_GROUPS; i++) {
    ...
    init_SF_tmap(args.SFA_tmap_list + i, SFA_list[idx].data_ptr(), M, K, BLOCK_K, CTA_GROUP);
    // the below assumes 1D TMA
  ...
  auto this_kernel = kernel_cutlass<NUM_GROUPS, BLOCK_K, N, K, NUM_STAGES, CTA_GROUP, epi_l2_mod>;
...
#define LAUNCH(G_, BK, N_, K_, CTA_GROUP, EPI_L2_MOD) \
  else if (G == G_ && N == N_ && K == K_) { \
    group_gemm_launch<G_, BK, N_, K_, CTA_GROUP, EPI_L2_MOD>(A_list, B_list, SFA_list, SFB_list, C_list); \
  }

  if (false) {}
  LAUNCH(8, 256, 4096, 7168, 2, L2_MOD::EVICT_LAST)
  LAUNCH(8, 256, 7168, 2048, 2, L2_MOD::EVICT_LAST)
  LAUNCH(2, 256, 3072, 4096, 2, L2_MOD::NONE)
  LAUNCH(2, 256, 4096, 1536, 2, L2_MOD::NONE)
