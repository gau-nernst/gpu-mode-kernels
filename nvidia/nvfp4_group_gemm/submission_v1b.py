...
struct Arguments {
  ...
  int N_list[NUM_GROUPS];
  int K_list[NUM_GROUPS];
};

template <int NUM_GROUPS, int NUM_STAGES>
__global__
...
void kernel_cutlass (const __grid_constant__ Arguments<NUM_GROUPS> args) {
  ...
  const int group_id = warp_uniform(blockIdx.z);

  const int M = args.M_list[group_id];
  const int N = args.N_list[group_id];
  const int K = args.K_list[group_id];

  // early exit
  if (bid_m >= cdiv(M, BLOCK_M) || bid_n >= N / BLOCK_N)
    return;
  ...
  if (warp_id == NUM_WARPS - 2) {
    ...
    for (int n = 0; n < BLOCK_N / WIDTH; n++) {
      float tmp[WIDTH];
      tcgen05_ld_32x32b<WIDTH>(tmp, warp_id * 32, n * WIDTH);
      ...
      if (row < M) {
        for (int i = 0; i < WIDTH / 16; i++)
          stg_16(C_ptr + row * N + col + i * 16, tmp + i * 16);
      }
    }
...
void group_gemm_launch(
  ...
  for (int i = 0; i < NUM_GROUPS; i++) {
    ...
    const int N = B_list[i].size(0);
    const int K = A_list[i].size(1) * 2;
  ...
  // launch max grid_m x grid_n to cover the largest group
  dim3 grid(max_N / BLOCK_N, cdiv(max_M, BLOCK_M), NUM_GROUPS);
  ...
  auto this_kernel = kernel_cutlass<NUM_GROUPS, NUM_STAGES>;
...
#define LAUNCH(NUM_GROUPS) \
  else if (G == NUM_GROUPS) { \
    group_gemm_launch<NUM_GROUPS>(A_list, B_list, SFA_list, SFB_list, C_list); \
  }
