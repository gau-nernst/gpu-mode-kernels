...
void combine(
  ...
  const int tb_size = NUM_WARPS * WARP_SIZE;
  constexpr int one_kernel = false;
...
TORCH_LIBRARY(p2p_module, m) {
  ...
  m.def("dispatch(Tensor pre_x, Tensor indices, Tensor(a!) send_counts, Tensor(b!) comm_x, Tensor(c!) comm_meta, Tensor(d!) comm_flag, Tensor(e!) comm_recv_counts, Tensor(f!) comm_recv_flag, Tensor(g!) post_x, Tensor(h!) post_meta, Tensor(i!) post_counts, int num_tokens, int dim, int topk, int num_experts, int max_num_tokens, int local_rank, Tensor heap_bases, int evt) -> ()");
  m.impl("dispatch", &dispatch);

  m.def("combine(Tensor post_y, Tensor post_meta, Tensor post_counts, Tensor(a!) comm_x, Tensor(b!) comm_flag, Tensor(c!) pre_y, Tensor weights, int num_tokens, int dim, int topk, int num_experts, int max_num_tokens, int local_rank, Tensor heap_bases, int evt) -> ()");
...
class P2PState:
    ...
    def dispatch(self, cfg: MoEConfig, pre_x: Tensor, indices: Tensor, evt: torch.cuda.Event | None = None):
        ...
        ops.dispatch(
            ...
            self.rank,
            self.heap_bases,
            evt.cuda_event if evt is not None else 0,
        )
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if False:
        ...
        dist.barrier()
        e[0].record()
        post_x, post_meta, post_counts = P2P_STATE.dispatch(cfg, rank_data.x, rank_data.indices, e[1])
        e[2].record()
        post_y = post_x
        P2P_STATE.combine(cfg, post_y, post_meta, post_counts, rank_data.weights, e[3])
        e[4].record()
