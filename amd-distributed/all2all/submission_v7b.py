...
template <int NUM_WARPS, int DIM, int MODE>
__global__
__launch_bounds__(NUM_WARPS * WARP_SIZE)
void combine_kernel(
...
#define my_dispatch(DIM) { \
  if constexpr (mode == 0) { \
    CUDA_CHECK(cudaLaunchKernel((void *)(combine_kernel<NUM_WARPS, DIM, 0>), grid, tb_size, kernel_args, 0, stream));\
    if (evt_i64 != 0) { cudaEvent_t evt = reinterpret_cast<cudaEvent_t>(evt_i64); CUDA_CHECK(cudaEventRecord(evt, stream)); } \
    CUDA_CHECK(cudaLaunchKernel((void *)(combine_kernel<NUM_WARPS, DIM, 1>), grid, tb_size, kernel_args, 0, stream));\
  } else if constexpr (mode == 1) { \
    if (evt_i64 != 0) { cudaEvent_t evt = reinterpret_cast<cudaEvent_t>(evt_i64); CUDA_CHECK(cudaEventRecord(evt, stream)); } \
    CUDA_CHECK(cudaLaunchKernel((void *)(combine_kernel<NUM_WARPS, DIM, 2>), grid, tb_size, kernel_args, 0, stream));\
  } else if constexpr (mode == 2) { \
    if (evt_i64 != 0) { cudaEvent_t evt = reinterpret_cast<cudaEvent_t>(evt_i64); CUDA_CHECK(cudaEventRecord(evt, stream)); } \
    CUDA_CHECK(cudaLaunchKernel((void *)(combine_kernel<NUM_WARPS, DIM, 3>), grid, tb_size, kernel_args, 0, stream));\
  } \
}
