...
void dispatch_kernel(
  ...
  auto do_send = [&](int bid, int num_blocks) {
    ...
    if (bid == 0) {
      ...
      // send to other ranks
      if (tid < WORLD_SIZE) {
        int *flag_addr = translate(comm_recv_counts, heap_bases[local_rank], heap_bases[tid]);
        flag_addr += local_rank;
        int flag = send_counts_smem[tid] + 1;
        __builtin_nontemporal_store(flag, flag_addr);
  ...
  auto do_recv = [&](int bid, int num_blocks) {
    ...
    if (tid == 0) {
      // spin_lock_system<64, false>(comm_recv_counts + src_rank);
      while (__builtin_nontemporal_load(comm_recv_counts + src_rank) == 0) {
        asm volatile("s_sleep %0;\n" :: "n"(64));
      }
    }
    __syncthreads();
    const int recv_count = comm_recv_counts[src_rank] - 1;
...
void dispatch(
  ...
  const int grid_size = 256;  // must be divisible by WORLD_SIZE
  const int tb_size = NUM_WARPS * WARP_SIZE;

  void *kernel_args[] = {(void *)&pre_x_ptr, (void *)&indices_ptr, (void *)&send_counts_ptr,
                         (void *)&comm_x_ptr, (void *)&comm_meta_ptr, (void *)&comm_flag_ptr, (void *)&comm_recv_counts_ptr,
...
#define my_dispatch(DIM) { \
  CUDA_CHECK(cudaLaunchKernel((void *)(dispatch_kernel<NUM_WARPS, DIM, true, true>), grid_size, tb_size, kernel_args, 0, stream)); \
...
TORCH_LIBRARY(p2p_module, m) {
  ...
  m.def("dispatch(Tensor pre_x, Tensor indices, Tensor(a!) send_counts, Tensor(b!) comm_x, Tensor(c!) comm_meta, Tensor(d!) comm_flag, Tensor(e!) comm_recv_counts, Tensor(f!) post_x, Tensor(g!) post_meta, Tensor(h!) post_counts, int num_tokens, int dim, int topk, int num_experts, int max_num_tokens, int local_rank, Tensor heap_bases) -> ()");
...
class P2PState:
    def __init__(self, rank: int, world_size: int, size: int = 10 << 30) -> None:
        ...
        self.dispatch_comm_recv_counts = self.malloc_symmetric((world_size,), dtype=torch.int32).zero_()
        # combine
    ...
    def dispatch(self, cfg: MoEConfig, pre_x: Tensor, indices: Tensor):
        ...
        self.dispatch_comm_recv_counts.zero_()  # reset
