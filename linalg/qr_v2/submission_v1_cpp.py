...
void qr176_kernel(const float *gA, float *gH, float *gTau) {
  ...
  for (int i = 0; i < 6; i++) {
    ...
    if (row < N)
      ldg_f32x8(x[i], gA + (row * N + warp_id * 8));
    else
      memset(x[i], 0, 8 * 4);
  }
  ...
  for (int panel = 0; panel < warp_id; panel++) {
    for (int i = 0; i < 8; i++) {
      ...
      float neg_tau = -sTau[col];  // negate
      float v[6][2];  // duplicate for f32x2 math later
      for (int j = 0; j < 6; j++) {
        const int row = j * 32 + lane_id;
        if (row < N)
          v[j][0] = v[j][1] = sV[col * N + row];
        else
          v[j][0] = v[j][1] = 0.0f;
      }
  ...
  for (int i = 0; i < 6; i++) {
    const int row = i * 32 + lane_id;
    if (row < N)
      stg_f32x8(gH + (row * N + warp_id * 8), x[i]);
  }
}
...
void qr176_launch(const float *gA, float *gH, float *gTau, int bs) {
  constexpr int N = 176;
  const int smem_size = (N * N + N) * 4 + N * 8;
  cudaFuncSetAttribute(qr176_kernel, cudaFuncAttributeMaxDynamicSharedMemorySize, smem_size);
  qr176_kernel<<<bs, 22 * 32, smem_size>>>(gA, gH, gTau);
}
"""
